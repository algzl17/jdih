...
function setNavActive () {
    ...
    sidemenuItems.forEach(e => {
        if (isValidUrl(e.getAttribute('href'))) {
            var url = new URL(e.getAttribute('href'))
            const jmlarray = currentPath.split('/')
            let myurl = currentPath
            if (jmlarray.length >= 3) {
                // jika segment url lebih dari 3
                myurl = currentPath.replaceAll('/', ',')
                myurl = myurl.match(/^(?:[^,]*,){1}([^,]*)/)[0]
                myurl = myurl.replaceAll(',', '/')
            }

            if (url.pathname === '/' + myurl) {
